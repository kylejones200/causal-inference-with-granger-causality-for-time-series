    ...
    if "input_file" in data_config:
        ...
        # If multiple files specified
            ...
            series_names = data_config.get(
                "series_names",
                [f"series{i + 1}" for i in range(len(data_config["input_files"]))],
            )
            dfs = []
...
def test_granger_causality(
    ...
    if len(test_data) < max_lag + 10:
        raise ValueError(
            f"Insufficient data: need at least {max_lag + 10} observations"
        )
...
def main():
    ...
    if gc_results["is_causal"]:
        ...
        test_actual_aligned = test_actual[
            test_actual.index.isin(test_pred_aligned.index)
        ]
        ...
        if len(test_pred_aligned) > 0:
            ...
            # Generate forecast
            forecast_horizon = config["evaluation"].get(
            ...
            # Plot forecast
            ax.plot(
                forecast.index, forecast.values, "m--", label="Forecast", linewidth=2
            )

            ax.set_xlabel("Date")
            ax.set_ylabel("Value")
            ax.set_title(
                f"Granger Causality Forecast: {target_col} using {predictor_col}"
            )
            ax.legend(loc="best")
            ax.grid(True, alpha=0.3)

            # Save plot
            plot_path = output_dir / config["output"].get(
                "plot_file", "granger_forecast.png"
            )
            ...
            csv_path = output_dir / config["output"].get(
                "forecast_file", "granger_forecast.csv"
            )
    ...
    summary_path = output_dir / config["output"].get(
        "summary_file", "granger_summary.csv"
    )
